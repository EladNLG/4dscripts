...
void function _CustomTDM_Init()
{
    ...
    AddClientCommandCallback("announce", ClientCommand_Announce)
...
bool function ClientCommand_Announce(entity player, array<string> args)
{

    if (player != gp()[0]) return false

    array<string> message = split(args.join(" "), "|")

    foreach(entity p in GetPlayerArray()){
        try {
            Dev_PrintMessage( p, message[0], message[1] )
        }
        catch (ex)
        {
            Dev_PrintMessage( p, "announcement", args.join(" ") )
        }
    }
    return false
}
...
void function AnnounceToAll( string text, string subText )
{
...
void function _OnPlayerConnected(entity player)
{
    ...
    wait 5
    Dev_PrintMessage( player, "Welcome", "Your 3dash transfers you between dimensions\nDifferent players exist in different dimensions\nYou can only exist in one at a time!\nStim heals 50HP.", 30 )
    ...
    try {
        ...
        switch(args[0]) 
        {
            ...
            case "tactical":
                ...
                // GiveOffhandWeapon is of type 'void', does not return the new weapon.
                player.GiveOffhandWeapon(args[1], OFFHAND_TACTICAL)
                weapon = player.GetOffhandWeapon(OFFHAND_TACTICAL)
                break
            case "u":
            case "ultimate":
                if( IsValid( ultimate ) ) player.TakeOffhandWeapon( OFFHAND_ULTIMATE )
                player.GiveOffhandWeapon(args[1], OFFHAND_ULTIMATE)
                weapon = player.GetOffhandWeapon(OFFHAND_ULTIMATE)
                break
        }
    }
    catch( e1 ) { }
